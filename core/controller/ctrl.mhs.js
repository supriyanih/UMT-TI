...
define([
    ...
    function Ctrmhs($scope,serviceAjax,localStorageService,growl){
        //set level
        $scope.level = localStorageService.get('user');
        $scope.gotoPage = function(page){
            page = page == 'berikutnya' ? parseInt($scope.page) + 1 : page;
            serviceAjax.getDataFromServer('mhs','get_all_bypaging',+page)
                .then(function(data){
                    if (data) {
                        $scope.mhs              = data['data'];
        ...
        $scope.gotoPage(1);
        $scope.modalMhs = function(action,id){
            if(action=='Tambah'){
                $scope.modaloption = 'show';
                $scope.action = action;
            }else if(action =='Edit'){
                serviceAjax.getDataFromServer('mhs','getbyid',+id).then(function(data){
                        if (data) {
                            $scope.modaloption = 'show';
                            $scope.action = action;
                            $scope.data = data[0];
        ...
        $scope.simpan = function(data){
            serviceAjax.posDataToServer('mhs','insert',data).then(function(data){
                if(data){
                    /*close modal*/
                    $scope.closeModal();
                    /*tamplikan list*/
        ...
        $scope.delete = function(id){
            serviceAjax.getDataFromServer('mhs','delete',+id).then(function(data){
                if(data.length > 0){
                    /*close modal*/
                    $scope.closeModal();
                    /*tamplikan list*/
        ...
        $scope.edit = function(data){
            serviceAjax.posDataToServer('mhs','update',data).then(function(data){
                if(data){
                    /*close modal*/
                    $scope.closeModal();
                    /*tamplikan list*/
